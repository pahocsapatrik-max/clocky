...
// Útvonal térkép a bejelentkezéshez
$routes = [
    'index.php' => 'bejelentkezes'
];

// Ha .php kiterjesztéssel hívják meg, átirányítjuk a magyarosított aliasra (bejelentkezes)
if (strpos($request_uri, '.php') !== false) {
    $pretty_name = $routes[$current_file] ?? 'bejelentkezes';
...
if (isset($_SESSION['logged_in'], $_SESSION['role']) && $_SESSION['logged_in'] === true) {
    ...
    if ($role === 1) {
        header('Location: kezdolap'); // Dashboard helyett kezdolap
        exit;
    } elseif ($role === 0) {
        header('Location: pult'); // Empdashboard helyett pult (vagy amit választasz)
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($result->num_rows === 1) {
            ...
            // Megjegyzés: A jövőben javasolt a password_hash() / password_verify() használata!
