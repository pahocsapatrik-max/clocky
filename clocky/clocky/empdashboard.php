...
// Útvonal térkép a fájlhoz (ha pl. ezt időmérőnek hívják)
$routes = [
...
// PHP kiterjesztés elrejtése és átirányítás szép URL-re
if (strpos($request_uri, '.php') !== false && !isset($_GET['api'])) {
    $pretty_name = $routes[$current_file] ?? 'kezdolap';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        elseif (isset($_POST['end_break']) && $active_work) {
            ...
            $stmt->bind_param("i", $wt_id);
            $stmt->execute();
            $status_update = "break_ended";
        }
        
        // RESTful API válasz kezelése
        if (isset($_GET['api'])) {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'success', 'action' => $status_update]);
            exit;
        }

        // Átirányítás tiszta URL-ekre
        $redirect = ($jogosultsag === 1) ? "kezdolap" : "pult";
